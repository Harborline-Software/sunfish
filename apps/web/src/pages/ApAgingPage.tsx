/**
 * ApAgingPage — cohort-3 AP aging report
 *
 * Run-on-demand AP aging report. Surfaces open payables by vendor and
 * by property as of a chosen date, plus a top-N overdue vendors list.
 *
 * Patterns exercised:
...
import { useApAgingSummary } from '@/hooks/useReports'
import {
  exportApAgingSummaryCsv,
  type ApAgingSummaryParameters,
  type ApAgingSummaryResult,
  type ApAgingSummaryRow,
  type TopOverdueVendor,
  type ChartId,
} from '@/api/reports'
...
interface ApAgingTotalsBarProps {
  totals: ApAgingSummaryRow
}

function ApAgingTotalsBar({ totals }: ApAgingTotalsBarProps) {
  const tiles = [
...
interface TopOverdueListProps {
  vendors: TopOverdueVendor[]
  topN: number
}

function TopOverdueList({ vendors, topN }: TopOverdueListProps) {
    ...
    <div className="rounded-lg border border-gray-200 bg-white overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100">
        <h2 className="text-lg font-semibold text-gray-900">
          Top {topN} overdue vendors
        </h2>
      </div>
      <ul className="divide-y divide-gray-100">
        {vendors.map((vendor, idx) => (
          <li key={vendor.vendorId} className="flex items-center gap-3 px-4 py-3">
            ...
            {/* Vendor name */}
            <span className="flex-1 text-sm font-medium text-gray-900">{vendor.vendorName}</span>
            {/* 90+ balance */}
            <span className="text-sm tabular-nums">
              {vendor.days90PlusBalance > 0 ? (
                <span>
                  <span className="text-xs text-gray-500 mr-1">90+:</span>
                  <span className="font-medium text-red-700">
                    <CurrencyAmount amount={vendor.days90PlusBalance} />
                  </span>
                </span>
              ) : (
                <span>
            ...
            <span className="text-sm tabular-nums text-gray-700">
              ...
              <CurrencyAmount amount={vendor.totalOpenBalance} />
            </span>
          </li>
        ))}
      </ul>
...
type ReportState =
  ...
  | { phase: 'READY_TO_RUN' }
  | { phase: 'LOADING' }
  | { phase: 'SUCCESS'; result: ApAgingSummaryResult; submittedParams: ApAgingSummaryParameters }
  | { phase: 'ERROR'; submittedParams: ApAgingSummaryParameters }
...
export function ApAgingPage() {
  ...
  const [topOverdueN, setTopOverdueN] = useState(10)

  const mutation = useApAgingSummary()
  ...
  const state: ReportState = (() => {
    ...
    if (mutation.isError) {
      return {
        phase: 'ERROR',
        submittedParams: mutation.variables as ApAgingSummaryParameters,
      }
    }
  ...
  function handleRun() {
    if (!chartId) return
    const params: ApAgingSummaryParameters = {
      chartId,
  ...
  async function handleExport(): Promise<void> {
    ...
    await exportApAgingSummaryCsv(submittedParams)
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Open payables by vendor and by property as of a chosen date
        </p>
      </div>
      ...
      >
        ...
        {/* As-of date */}
        <div className="flex flex-col gap-1">
          <label htmlFor="ap-as-of-date" className="text-xs font-medium text-gray-700">
            As of
          </label>
          <input
            id="ap-as-of-date"
        ...
        {/* Top-N overdue stepper */}
        <div className="flex flex-col gap-1">
          <label htmlFor="ap-top-n" className="text-xs font-medium text-gray-700">
            Top overdue
          </label>
      ...
      {/* Loading state */}
      {state.phase === 'LOADING' && (
        <div className="space-y-6" aria-busy="true" aria-label="Loading AP aging report">
          <SkeletonTotalsBar />
      ...
      {state.phase === 'ERROR' && (
        <ErrorSurface
          variant="retryable"
          title="Couldn't run the AP aging report"
      ...
      {state.phase === 'SUCCESS' && (() => {
        ...
        if (isEmpty) {
          return (
            <div className="rounded-lg border border-gray-200 bg-white p-8 text-center">
              <h2 className="text-base font-medium text-gray-900">No outstanding payables.</h2>
              <p className="mt-1 text-sm text-gray-600">
                All vendor bills are current as of {result.asOf.slice(0, 10)}.
              </p>
            </div>
          )
        }
        ...
        return (
          <div className="space-y-6">
            ...
            <ApAgingTotalsBar totals={result.totals} />

            {/* By Vendor section */}
            <section>
              <h2 className="text-lg font-semibold text-gray-900 mt-6 mb-3">By Vendor</h2>
              <AgingTable
                rows={result.byVendor}
            ...
            {/* Top overdue vendors — hidden when topOverdueN is 0 or no results */}
            {topOverdueN > 0 && result.topOverdue.length > 0 && (
              <TopOverdueList
                vendors={result.topOverdue}
                topN={topOverdueN}
              />
            )}
          </div>
